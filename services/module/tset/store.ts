...
import type * as mT from '../../types/tset/store'
...
placeOrder: mT.PlaceOrder = body => {
  ...
  const config = { '/store/order',body: body, method: 'post' }
  return this.request(config)
}


/**
 * @name tsetGetInventory
...
getInventory: mT.GetInventory = () => {
  ...
  const config = { '/store/inventory', method: 'get' }
  return this.request(config)
}


/**
 * @name tsetGetOrderById
...
getOrderById: mT.GetOrderById = orderId => {
  ...
  const config = { `/store/order/${orderId}`, method: 'get' }
  return this.request(config)
}


/**
 * @name tsetDeleteOrder
...
deleteOrder: mT.DeleteOrder = orderId => {
  const url = `/store/order/${orderId}`
  const config = { `/store/order/${orderId}`, method: 'delete' }
  return this.request(config)
}
}
