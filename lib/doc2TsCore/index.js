...
class Doc2Ts {
    ...
    generateFile() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const filePathList = [];
            const allProcess = this.StandardDataSourceList.map((i) => __awaiter(this, void 0, void 0, function* () {
                const { data, name } = i;
    ...
    transform2js() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!isJs)
                return;
            try {
                const outDirPath = path_1.default.join((0, utils_1.resolveOutPath)(outDir), 'index.ts');
                const filePath = (0, utils_1.findDiffPath)(path_1.default.join(__dirname, '../../'), outDirPath);
                log_1.default.clear();
                log_1.default.info('正在转换 ts 文件为 js');
                const cmd = `tsc ${filePath} --target esnext --module es6 ${declaration ? '--declaration' : ''} --skipLibCheck`;
                (0, child_process_1.execSync)(cmd).toString();
    ...
    remveTsFile() {
        ...
        const dirPath = path_1.default.join(outDirPath, 'module');
        const indexFilePath = path_1.default.join(outDirPath, 'index.ts');
        if (fs_1.default.existsSync(indexFilePath))
            fs_1.default.unlinkSync(indexFilePath);
        (0, utils_1.traverseDir)({
            dirPath,
            callback(fileInfo) {
                const { filePath } = fileInfo;
                if (removeTypeReg.test(filePath))
