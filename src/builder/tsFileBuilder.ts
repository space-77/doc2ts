...
// function generatorApiMethod(docApi: DocApi, funcItem: PathItem, options: { typesList: string[]; config: Config }) {
//   let content = ''
//   const { typesList, config } = options
...
//   const desc = getDesc({ description, deprecated, externalDocs, summary }, { paramList, returnType: returnTypeStrName })
...
//     content += generatorApiMethod(docApi, funcItem, { typesList, config })
//   }
//   content += '}'
...
export default class TsFileBuilder extends Base {
  constructor(doc: DocListItem, config: Config) {
    super(doc, config)
  }
  ...
  private generatorApiMethod(funcItem: PathItem, typesList: string[]) {
    ...
    const { item, name, method, apiPath } = funcItem
    const { disableParams, arrowFunc, declaration } = config
    const { responseType, parameterType, requestBodyType } = funcItem
    const { deprecated, description, externalDocs, summary } = item
